...
class Login extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('_partials/login/header.php');
            $this->load->view('dashboard/login/index.php', $data);
            $this->load->view('_partials/login/footer.php');
        } else {
                ...
                foreach ($check->result() as $ck) {
                    if ($ck->status == 1) {
                        ...
                        if ($sess_data['level'] == 'admin') {
                            ...
                            redirect('guru');
                        } elseif ($sess_data['level'] == 'kepala sekolah') {
                            redirect('kepala');
