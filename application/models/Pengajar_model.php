...
class Pengajar_model extends CI_Model
{
    ...
    {
        //mysql
        // $this->db->select('tp.id_pengajar, tp.jabatan, tg.nama_guru as guru, CONCAT_WS(" / ", tm.nama_mapel, tm.level) as mapel, tk.kelas, tt.nama as tahun');

        //sqlite
        $this->db->select("tp.id_pengajar, tp.jabatan, tg.nama_guru as guru, tm.nama_mapel ||' /'|| tm.level as mapel, tk.kelas, tt.nama as tahun");

        $this->db->from('tb_pengajar tp');
        $this->db->join('tbl_guru tg', 'tp.id_guru = tg.id_guru', 'left');
        $this->db->join('tbl_matapelajaran tm', 'tp.id_mapel = tm.id_mapel', 'left');
    ...
    {
        $this->db->select('tp.id_pengajar, tg.id_guru, tg.nama_guru');
        $this->db->from('tbl_guru tg');
        $this->db->join('tb_pengajar tp', 'tp.id_guru = tg.id_guru', 'inner');
        $this->db->join('tbl_kelas tk', 'tk.id_kelas = tp.id_kelas', 'inner');
        $this->db->join('tbl_matapelajaran tm', 'tm.id_mapel = tp.id_mapel', 'inner');
    ...
    public function get_mapel_pengampu($id_guru, $id_kelas = NULL, $id_mapel = NULL, $id_tahun = NULL)
    {
        $this->db->select('*');
        $this->db->from('tb_pengajar tp');
        $this->db->join('tbl_kelas tk', 'tk.id_kelas = tp.id_kelas', 'left');
        $this->db->join('tbl_matapelajaran tm', 'tm.id_mapel = tp.id_mapel', 'left');
    ...
    var $column_order = array(null, 'tg.nama_guru', 'tp.jabatan', 'tm.nama_mapel', 'tm.level', 'tk.kelas', 'tahun', 'tt.semester'); //Sesuaikan dengan field
    var $column_search = array('tg.nama_guru'); //field yang diizin untuk pencarian 
    var $order = array('kelas' => 'asc'); // default order 

    private function _get_datatables_query()
    {
        $this->db->select('tp.id_pengajar, tg.nama_guru, tp.jabatan, tm.nama_mapel, tm.level, tk.kelas, tt.nama as tahun, tt.semester');
        $this->db->from('tb_pengajar tp');
        $this->db->join('tbl_guru tg', 'tp.id_guru = tg.id_guru', 'left');
        $this->db->join('tbl_matapelajaran tm', 'tp.id_mapel = tm.id_mapel', 'left');
        $this->db->join('tbl_kelas tk', 'tp.id_kelas  = tk.id_kelas', 'left');
